...
class EvaluatedModelMapper(private val input: ReporterInput) {
            ...
            project.scopes.forEach { scope ->
                scope.getShortestPaths().forEach { (id, path) ->
                    ...
                    val packagePath = EvaluatedPackagePath(
                        pkg = pkg,
                        project = packages.find { it.id == project.id }!!,
                        scope = scopes.addIfRequired(ScopeName(scope.name)),
                        path = path.map { parentId -> packages.find { it.id == parentId }!! }
                    )
    ...
    private fun addDependencyTree(project: Project, pkg: EvaluatedPackage) {
        ...
        val scopeTrees = project.scopes.map { scope ->
            var totalNodes = scope.dependencies.size
            fun PackageReference.walk() {
                totalNodes += dependencies.size
                dependencies.forEach { it.walk() }
            }
            scope.dependencies.forEach { it.walk() }
